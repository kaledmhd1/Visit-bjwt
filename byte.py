...
x = [f'{i:02x}' for i in range(1, 128)]

# تشفير ID إلى HEX مشفر خاص بـ Free Fire
def Encrypt_ID(uid: int) -> str:
    final_result = ""
    while uid > 0:
        value = uid % 128
        uid = uid // 128
        final_result += x[value]
    final_result += dec[uid % 128]
    return final_result

# فك تشفير ID من HEX إلى رقم UID
def Decrypt_ID(da):
    if da is None:
        return None
    ...
    if len(da) == 10:
        ...
        try:
            ...
            return "Invalid hex value in input"

    elif len(da) == 8:
        w = 128
        for _ in range(2):  # 8 / 2 - 1 = 3 - 1 = 2
            w *= 128
        x1 = da[:2]
        x2 = da[2:4]
        x3 = da[4:6]
...
# تشفير بيانات API باستخدام AES CBC
def encrypt_api(data: str) -> str:
...
if __name__ == "__main__":
    uid = 270279853
    hex_id = Encrypt_ID(uid)
    print("Encrypted ID:", hex_id)

    decrypted_uid = Decrypt_ID(hex_id)
    print("Decrypted UID:", decrypted_uid)

    encrypted_packet = encrypt_api("f2212101")
    print("Encrypted Packet:", encrypted_packet)                y= (x-int(strx))*128
                stry =str(int(y))
                z = (y-int(stry))*128
                strz =str(int(z))
                n =(z-int(strz))*128
...
print(Encrypt_ID(270279853))        final_result = "0" + final_result
        return final_result
    else:
        return final_result
