...
def handle_visit(player_id: int, token: str):
    try:
...
async def visit_profile(player_id: int = None, token: str = None):
    if not player_id or not token:
        raise HTTPException(status_code=400, detail="player_id and token are required in query params")

    executor.submit(handle_visit, player_id, token)

    return JSONResponse(content={"status": "processing", "message": f"Visit request for {player_id} is being processed"}, status_code=202)
